...
\f0\fs24 \cf0 const CSV_PATH = 'Events_Database.csv';\
const els = \{\
...
\
  const featureCard = r => `\
    <div class="small" style="padding:8px 0;border-bottom:1px solid rgba(220,226,238,0.7)">\
      ...
      $\{escapeHtml(displayDate(r.Date))\} \'b7 $\{escapeHtml(r.Venue)\}\
...
async function main() \{\
  ...
  els.total.textContent = `$\{rows.length.toLocaleString()\} events`;\
  initFilters();\
  buildFeatures();\
  attachEvents();\
  render();\
